...
def register_base_impl(obj):
    """
    装饰器：自动将类或函数名添加到当前模块的 __all__ 列表中。
    """
    # 获取定义该对象的模块
    module = sys.modules[obj.__module__]
    
    # 获取模块的 __all__，如果不存在则初始化为空列表
...
class ProviderRegistry:
    ...
    @classmethod
    def register_vendor_impl(cls, op_name, provider_name):
        def decorator(wrapped_class):
            if op_name not in cls.OP_MAPPING:
                cls.OP_MAPPING[op_name] = {}
            cls.OP_MAPPING[op_name][provider_name] = wrapped_class
